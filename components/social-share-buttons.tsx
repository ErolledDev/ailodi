        ...
        {shareButtons.map((button) => {
          ...
          return (
            <button
              ...
              className={`inline-flex items-center justify-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-2 sm:py-2.5 rounded-lg font-medium transition-all duration-200 text-xs sm:text-sm ${button.className} hover:scale-105 active:scale-95`}
              aria-label={button.ariaLabel}
            >
              <IconComponent size={14} className="sm:w-4 sm:h-4" />
              <span className="hidden sm:inline truncate">{button.name}</span>
            </button>
          );
        })}
      </div>
